...
def toc_transformer(toc_content, model=None):
    ...
    init_prompt = """
You are given a table of contents.

Your job is to transform the entire table of contents into valid JSON format.
...
"""
    ...
    # IMPORTANT: Make sure your API call uses temperature=0
    response = ChatGPT_API(model=model, prompt=prompt)

    # Safe JSON parsing
    parsed = safe_json_load(response)

    if "table_of_contents" not in parsed:
        raise ValueError("Model response missing 'table_of_contents' key")
